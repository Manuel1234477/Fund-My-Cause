...
jest.mock("@/components/ui/WalletSelectModal", () => ({
  WalletSelectModal: ({
    ...
    onSelect: (w: "freighter" | "lobstr") => void;
  }) => (
...
beforeEach(() => {
  ...
  useWalletStore.setState(INITIAL_STATE, true);
});
...
describe("useWallet", () => {
  ...
  it("sets address when Freighter is selected", async () => {
    ...
    await act(async () => {
      screen.getByRole("button", { name: "select-freighter" }).click();
    });
  ...
  it("sets address when LOBSTR is selected", async () => {
    ...
    await act(async () => {
      screen.getByRole("button", { name: "select-lobstr" }).click();
    });

    await waitFor(() =>
      expect(screen.getByTestId("address")).toHaveTextContent("GLOBSTR123"),
    );
  });
  ...
  it("shows xlmBalance from useXlmBalance when connected", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByTestId("xlm-balance")).toHaveTextContent("42.50"),
    );
  });

  it("sets error when adapter connect throws", async () => {
    mockFreighterConnect.mockRejectedValue(
